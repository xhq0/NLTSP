...
from .cost_model import RandomModel, XGBModel, MLPModel, LGBModel, TabNetModel,  NLTSPModel
...
def make_search_policies(
    ...
    if isinstance(search_policy, str):
        ...
        if model_type in ['xgb', 'xgb-no-update', 'mlp', 'mlp-no-update', 'tab', 'tab-no-update',  'nltsp-no-update']:
            ...
            elif model_type in ['nltsp-no-update']:
                cost_model = NLTSPModel(target=tasks[0].target)
            else:
                cost_model = MLPModel(
                    disable_update=disable_cost_model_update,
                    few_shot_learning=few_shot_learning
