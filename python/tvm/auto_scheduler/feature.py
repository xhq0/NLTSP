...
def get_per_store_features_from_states_nltsp(
    states: List[Union[State, StateObject]], task: "SearchTask", max_n_bufs: Optional[int] = None):
    if isinstance(states[0], State):
        state_objects = [s.state_object for s in states]
    elif isinstance(states[0], StateObject):
        state_objects = states
    byte_arr = _ffi_api.GetPerStoreFeaturesFromStatesNLTSP(
        state_objects, task, max_n_bufs or DEFAULT_MAX_N_BUFS
    )

    segment_sizes = np.frombuffer(byte_arr, dtype = np.int32, count = len(state_objects))
    features = np.frombuffer(byte_arr, dtype = np.float32, offset = len(state_objects) * np.dtype(np.float32).itemsize)
    # nparr = np.frombuffer(byte_arr, dtype = np.float32)
    # nparr.resize(len(state_objects) * 8, 10, 9)

    return segment_sizes, features
