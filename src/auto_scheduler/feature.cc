...
std::map<std::string, int> OpNameMap = {
  {"Conv2dOutput", 0},
	{"Conv2dOutput.local", 1},
	{"Conv2dOutput.repl", 2},
	{"Conv2dOutput.rf", 3},
	{"Conv3dOutput", 4},
	{"DepthwiseConv2d", 5},
	{"DepthwiseConv2d.local", 6},
	{"PadInput", 7},
	{"PadInput.shared", 8},
	{"PaddedInput", 9},
	{"PaddedInput.shared", 10},
	{"T_add", 11},
	{"T_dense", 12},
	{"T_dense.local", 13},
	{"T_divide", 14},
	...
	{"T_multiply", 16},
	{"T_multiply_red", 17},
	{"T_multiply_red.repl", 18},
	{"T_multiply_red.rf", 19},
	{"T_relu", 20},
	{"T_softmax_exp", 21},
	{"T_softmax_expsum", 22},
	{"T_softmax_expsum.repl", 23},
	{"T_softmax_expsum.rf", 24},
	{"T_softmax_maxelem", 25},
	{"T_softmax_maxelem.repl", 26},
	{"T_softmax_maxelem.rf", 27},
	{"T_softmax_norm", 28},
	{"T_tanh", 29},
	{"bgemm", 30},
	{"bgemm.local", 31},
	...
	{"pad_temp", 43},
	{"pad_temp.shared", 44},
	{"placeholder.d.shared", 45},
	{"placeholder.shared", 46},
	{"placeholder_red", 47},
	{"placeholder_red.repl", 48},
	{"placeholder_red.rf", 49},
	{"tensor", 50}
};

std::map<int, float> powers_of_two = {
    {0, 0.0f},
    {1, 0.0f},
    {2, 1.0f},
    {4, 2.0f},
...
void branching_to_linear(
    const std::vector<std::array<float, N>>& branching_loop,
    const std::vector<int>& depth,
    std::vector<std::array<std::array<float, N>, M>> *linear_loops) {
	  linear_loops->reserve(8);
	  auto* linear_ptr = linear_loops->data();
    std::array<std::array<float, N>, M> new_element;
    memset(&new_element, 0, sizeof(new_element));
    
    linear_loops->push_back(new_element);
	size_t ptr_linear_loop = 0;
	size_t ptr_axis = 0;
	for (size_t i = 0; i < branching_loop.size() - 1; ++i) {
		if(ptr_axis < M) {
			linear_ptr[ptr_linear_loop][ptr_axis] = branching_loop[i];
		}
		ptr_axis++;
		if (depth[i] >= depth[i + 1]) {
			linear_loops->push_back(new_element);
			size_t copy_len = std::min<size_t>(depth[i + 1], M);
            for (size_t j = 0; j < copy_len; ++j) {
                linear_ptr[ptr_linear_loop + 1][j] = linear_ptr[ptr_linear_loop][j];
            }
			ptr_linear_loop ++;
			ptr_axis = depth[i + 1];
		}
	...
	if(ptr_axis < M) {
		linear_ptr[ptr_linear_loop][ptr_axis] = branching_loop.back();
	}
}


void StageFeatureNLTSP(
    std::vector<std::array<float, N>>& branching_loop,
	...
	std::array<float, N> axis_fea;
	const Stage& stage = state->stages[stage_id];
  float axis_fea_2 = OpNameMap[stage->op->name] + 1;
  float axis_fea_3 = powers_of_two[stage->attrs.auto_unroll_max_step];
  float axis_fea_4 = stage->attrs.storage_offset;
	...
	for (size_t i = 0; i < stage->iters.size(); ++i) {
		...
		if (!(delete_trivial_loop && iter->range.defined() && is_one(iter->range->extent))) {
			size_t total_indent = base_indent + indent;
			depth.push_back(total_indent);

      axis_fea[0] = static_cast<int>(iter->annotation) + 1;
      axis_fea[1] = static_cast<int>(iter->iter_kind) + 1;
      axis_fea[2] = axis_fea_2;
      axis_fea[3] = axis_fea_3;
			axis_fea[4] = axis_fea_4;

      axis_fea[5] = 0;
			if (iter->range.defined()) {
        if (const IntImmNode* intNodePtr = iter->range->extent.as<IntImmNode>()) {
          axis_fea[5] = intNodePtr->value;
        } 
      } 

			std::string iter_name = iter->name;
			axis_fea[6] = (iter_name.find('@') != std::string::npos) ? 1 : 0;
			if (iter_name.length() == 1 || iter_name[iter_name.length() - 2] != '.') {
				axis_fea[7] = 0;
        axis_fea[8] = 0;
			} else {
        axis_fea[7] = 1;
				axis_fea[8] = iter_name.back() - '0';
			}

			branching_loop.push_back(axis_fea);
			indent += 1;
		}
		...
		if (state.defined()) {
			...
			if (pair != state->attach_map->iter_to_attached_stages.end()) {
				...
				for (const auto& attach_stage_id : pair->second) {
					StageFeatureNLTSP(branching_loop, depth, attach_stage_id, state, base_indent + indent, delete_trivial_loop);
				}
			}
		}
	}
...
void GetPerStoreFeaturesWorkerFuncNLTSP(const SearchTask& task, const State& state, int max_n_bufs,
                                        std::vector<std::array<std::array<float, N>, M>> *linear_loops, std::atomic<int>* error_ct) {
                                       

	bool delete_trivial_loop = true;

	std::vector<std::array<float, N>> branching_loop;
	std::vector<int> depth;
	branching_loop.reserve(60);
	depth.reserve(60);
	...
	for (size_t i = 0; i < state->stages.size(); ++i) {
		...
		if (stage->op_type == StageKind::kPlaceholder) {
			...
			if (stage->compute_at == ComputeAtKind::kRoot) {
				StageFeatureNLTSP(branching_loop, depth, i, state, 0, delete_trivial_loop);
			}
		} else {
	...
	branching_to_linear(branching_loop, depth, linear_loops);
...
void GetPerStoreFeaturesFromStatesNLTSP(const Array<State>& states, const SearchTask& task,
                                   int skip_first_n_feature_extraction, int max_n_bufs,
                                   std::vector<std::vector<std::array<std::array<float, N>, M>>>* features) {
                                   
                                    
  features->assign(states.size(), std::vector<std::array<std::array<float, N>, M>>());
  ...
  support::parallel_for(skip_first_n_feature_extraction, states.size(),
                        [&task, &states, &max_n_bufs, &features, &error_ct](int i) {
                          GetPerStoreFeaturesWorkerFuncNLTSP(task, states[i], max_n_bufs,
                                                        &(*features)[i], &error_ct);
                        });
}
...
TVMByteArray SerializeFeaturesNLTSP(std::vector<std::vector<std::array<std::array<float, N>, M>>>&& features,
                                    std::vector<char>* out_data) {
    size_t feature_size = features.size();
    std::vector<int> segment_sizes;
    segment_sizes.reserve(feature_size);
    for (size_t i = 0; i < feature_size; i++) {
        segment_sizes.push_back(features[i].size());
    }
    size_t total_bytes = feature_size * sizeof(int) + std::accumulate(segment_sizes.begin(), segment_sizes.end(), 0) * M * N * sizeof(float);

    out_data->resize(total_bytes); // Resize directly to avoid reallocations
    char* ptr = out_data->data();
    // Copy segment_sizes directly into out_data
    std::memcpy(ptr, segment_sizes.data(), feature_size * sizeof(int));
    ptr += feature_size * sizeof(int);

    // Flatten and copy features
    for (auto& fec : features) {
        for (auto& arr : fec) {
            std::memcpy(ptr, arr.data(), M * N * sizeof(float));
            ptr += M * N * sizeof(float);
        }
    }

    assert(ptr - out_data->data() == total_bytes);
    return TVMByteArray{out_data->data(), total_bytes};
}
...
TVM_REGISTER_GLOBAL("auto_scheduler.GetPerStoreFeaturesFromStatesNLTSP")
.set_body([](TVMArgs args, TVMRetValue* ret) {
    Array<State> states = args[0];
    SearchTask task = args[1];
    int max_n_bufs = args[2];
	...
	std::vector<std::vector<std::array<std::array<float, N>, M>>> features;
    GetPerStoreFeaturesFromStatesNLTSP(states, task, 0, max_n_bufs, &features);
    std::vector<char> byte_data; 
    *ret = SerializeFeaturesNLTSP(std::move(features), &byte_data);
});
